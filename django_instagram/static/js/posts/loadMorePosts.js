  ...
  try {
    ...
    data.posts.forEach(post => {
      ...
      <article class="w-3/4 border rounded-lg border-gray-300 mx-auto shadow-lg bg-white">
        ...
        <!-- 댓글 입력 폼 -->
        <div class="mt-5 p-4">
          <form action="/posts/${post.id}/comment_create/" method="post" class="flex flex-col space-y-4">
            <input type="hidden" name="csrfmiddlewaretoken" value="${post.csrf_token}">
            <textarea name="contents" class="p-2 border rounded-lg w-full" placeholder="댓글을 입력하세요"></textarea>
