...
const loadMorePosts = async () => {
  ...
  try {
    ...
    data.posts.forEach(post => {
      const postElement =postsHtmlTemplate(post);     
...
async function commentCreate(postId, csrfToken, event) {
  ...
  try {
    ...
    if (data.success) {
      ...
      commentListElementContainer.insertAdjacentHTML('afterbegin', commentListElement(data.comment, csrfToken));
      form.reset();
    }
        ...
        <article class="w-3/4 border rounded-lg border-gray-300 mx-auto shadow-lg bg-white" id="article-${post.id}">
          ...
          <div class="border-t border-gray-200 p-4 mt-5">
            ...
            <div class="comment-list">
              ${post.comment_post.map(comment => commentListElement(comment, post.csrf_token) ).join('')}
            </div>
          </div>
...
//댓글 목록 템플릿
const commentListElement = (comment, csrf_token) => {
